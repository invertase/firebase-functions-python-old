...
from firebase_functions.pubsub import (
    Message,
...
def on_message_published_function(event: CloudEvent[MessagePublishedData]):
    """Test on message published function"""
    ...
    assert isinstance(event.data.message, Message), "Event data is a Message object"
    assert isinstance(event.data.message.json, dict), "Event data is a Message object"
    assert event.data.message.json == {"data": "Hello world"}, "Message data is a dict"
...
def on_message_published_function_no_message_data(
    ...
    """Test on message published function no message data"""

    assert (
        event.data.message.json is None
    ), "Message data is None when no data is passed in the event"

    message_as_dict = event.data.message.asdict()

    assert "attributes" in message_as_dict, "Message has attributes"
    assert "ordering_key" in message_as_dict, "Message has ordering key"
...
def on_message_published_function_data_is_not_parsable(
    ...
    """Test on message published function data is not parsable"""

    with pytest.raises(Exception):
        assert isinstance(
            event.data.message.json, dict
        ), "The message data is not parsable to json"


triggers = {
    "on_message_published_function": on_message_published_function,
...
def test_trigger_pubsub():
    ...
    with serve_triggers(triggers=triggers).test_client() as client:
        ...
        assert res_call.status_code == 200, "Response status code is 200"
...
def test_trigger_pubsub_data_is_not_parsable():
    """Test trigger pubsub data is not parsable"""
